        ...
        try:
            if row[0] != 'OS_timestamp' and (len(row)-1) == no_columns_csv:
                ...
                if float(row[1]) < 60:
                    ...
                    if time.time() - cf.game_start_time > 20:
                        pt.parse_decision_tree(cf.data, initial_state)
                    else:
                        cf.status['Phase'].append(cf.data['Phase'])
                        cf.status['Hunger_AVATAR'].append(int(float(cf.data['Hunger_AVATAR'])))
                        cf.status['Health_AVATAR'].append(int(float(cf.data['Health_AVATAR'])))
                        cf.status['Sanity_AVATAR'].append(int(float(cf.data['Sanity_AVATAR'])))
                        cf.status['Curr_Equip_Hands_AVATAR'].append(cf.data['Curr_Equip_Hands_AVATAR'])
                        cf.status['Attack_Target_AVATAR'].append(cf.data['Attack_Target_AVATAR'])
                        cf.status['Defense_Target_AVATAR'].append(cf.data['Defense_Target_AVATAR'])
                        cf.status['Food_AVATAR'].append(cf.data['Food_AVATAR'])
                        cf.status['Twigs_AVATAR'].append(cf.data['Twigs_AVATAR'])
                        cf.status['Flint_AVATAR'].append(cf.data['Flint_AVATAR'])
                        cf.status['Grass_AVATAR'].append(cf.data['Grass_AVATAR'])
                        cf.status['Log_AVATAR'].append(cf.data['Log_AVATAR'])
                        cf.status['Twigs_AVATAR'].append(cf.data['Twigs_AVATAR'])
                        cf.status['Rock_AVATAR'].append(cf.data['Rock_AVATAR'])
                        cf.status['Is_Light_AVATAR'].append(cf.data['Is_Light_AVATAR'])
                        cf.status['Is_Fireplace_AVATAR'].append(cf.data['Is_Fireplace_AVATAR'])
                        cf.status['Is_Light_AVATAR'].append(cf.data['Is_Light_AVATAR'])
                        cf.status['Curr_Equip_Hands_AVATAR'].append(cf.data['Curr_Equip_Hands_AVATAR'])
                        cf.status['Is_Monster_AVATAR'].append(cf.data['Is_Monster_AVATAR'])
                        cf.status['Player_Xloc'].append(float(cf.data['Player_Xloc']))
            else:
                pass

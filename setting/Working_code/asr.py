...
def asr_tts_excel():
    ...
    with open(cf.script_path, encoding="utf-8-sig", newline='', mode=mode) as f:
        ...
        if cf.script_counter == 1:
            writer.writerow(['Speaker', 'Script', 'Start', 'Finish', 'Self_ASR', 'Sim_score'])
            cf.script_counter += 1
...
class listen_micr:
    ...
    def respond_to_user_utt(self, user_text, language):
        try:
            ...
            cf.sim_score = util.dot_score(query_emb, response_emb)[0].tolist()[0]
            
            if cf.sim_score > cf.threshold:
                tts.synthesize_utt(decoded_text, 'asr')
            else:
                cf.list_utter.extend(["Avatar", decoded_text, str(datetime.timedelta(seconds=cf.utt_start_time)).split(".")[0]])
                cf.utt_finish_time = round(time.time() - cf.game_start_time, 5)
                cf.list_utter.append(str(datetime.timedelta(seconds=cf.utt_finish_time)).split(".")[0])
                
                cf.list_utter.append('under threshold')
                cf.list_utter.append(cf.sim_score)
                asr_tts_excel()
                cf.list_utter = []
